#ifndef IFR_ROS2_CV__PACKAGE_RM_BUFF__MEM_MAP__H
#define IFR_ROS2_CV__PACKAGE_RM_BUFF__MEM_MAP__H
#include <cstddef>
...
namespace buff_tracker {
    class MemMapPool {
#define RAF_MMP_LOG(type, ...) RCLCPP_##type##_STREAM(node->get_logger(), "[MemMapPool] " << __VA_ARGS__)
#define RAF_MMP_LOG_CUDA(expr, err)                                                                          \
    do {                                                                                                     \
        ...
        if (result == cudaError::cudaSuccess) {                                                              \
            if constexpr (DEBUG) RAF_MMP_LOG(DEBUG, (#expr " = ") << result);                                \
        } else                                                                                               \
            RAF_MMP_LOG(ERROR, (#expr " = ") << result                                                       \
                                             << ", " << err                                                  \
                                             << " (" << cudaGetErrorName(result) << ")"                      \
                                             << "->" << cudaGetErrorString(result));                         \
    } while (0)
        std::vector<void *> cpu_ptrs;            ///< CPU指针保存
        std::unordered_map<intptr_t, void *> mem;///< CPU内存到GPU内存的映射
        rclcpp::Node *node;                      ///< 所属节点

        static inline intptr_t toKey(const void *const ptr) { return reinterpret_cast<intptr_t>(ptr); }

    public:
        MemMapPool(rclcpp::Node *node) : node(node) {}
        ~MemMapPool() {
            for (const auto &x: mem) {
                RAF_MMP_LOG(DEBUG, "Unregister host memory " << x.second);
        ...
        void *getDevicePointer(void *host_ptr, size_t size, bool readonly = false) {
            ...
            if (dev_ptr != nullptr) {
                ...
                if constexpr (DEBUG) RAF_MMP_LOG(DEBUG, "Get device memory" << dev_ptr);
                return dev_ptr;
            }
        }

        void malloc(void **cpu, void **gpu, size_t size) {
            *cpu = ::malloc(size);
            CV_Assert(*cpu != nullptr);
            cpu_ptrs.emplace_back(*cpu);
            *gpu = getDevicePointer(*cpu, size);
        }

        ///@brief 分配CPU与GPU共享的内存
        void malloc(cv::Mat &cpu, cv::cuda::GpuMat &gpu, cv::Size size, int type) {
            if (cpu.empty() || cpu.size() != size) {
...
#endif// IFR_ROS2_CV__PACKAGE_RM_BUFF__MEM_MAP__H
